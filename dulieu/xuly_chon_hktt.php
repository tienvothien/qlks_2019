<?php 	
	...
	if (isset($_POST['id_nhan_vien_sua'])) {// hiện tt xã sẽ thay đổi sửa tt nhân viên
		...
		while ($rowxa = mysqli_fetch_array($queryxa)) {
			...
			$maxa = $rowxa['maxa'];
			echo "<option value='$maxa'>$capxa $tenxa</option>";}
	}// end hiện tt xã sẽ thay đổi sửa tt nhân viên
	if (isset($_POST['id_nhan_vien_sua_huyen'])) {// hiện tt xã sẽ thay đổi sửa tt nhân viên
	...
	if (isset($_POST['id_khach_hang_sua'])) {// hiện tt xã sẽ thay đổi sửa tt khách hàng
		$id_khach_hang_sua = $_POST['id_khach_hang_sua'];
		include 'conn.php';
		$sqlxa = "SELECT xa.maxa, xa.capxa, xa.tenxa FROM khachhang , huyen, xa WHERE khachhang.id='$id_khach_hang_sua' AND khachhang.mahuyen= huyen.mahuyen AND huyen.mahuyen=xa.mahuyen ORDER by xa.capxa,xa.tenxa ASC";
		$sqlxahien = mysqli_fetch_array(mysqli_query($conn,"SELECT xa.maxa, xa.capxa, xa.tenxa FROM khachhang , xa WHERE khachhang.id='$id_khach_hang_sua' AND khachhang.maxa= xa.maxa ORDER by xa.capxa, xa.tenxa ASC"));
		$queryxa = mysqli_query($conn, $sqlxa);
		echo " <option value='".$sqlxahien['maxa']."'>".$sqlxahien['capxa']. $sqlxahien['tenxa']."</option>";
		while ($rowxa = mysqli_fetch_array($queryxa)) {
			$tenxa = $rowxa['tenxa'];
			...
			$maxa = $rowxa['maxa'];
			echo "<option value='$maxa'>$capxa $tenxa</option>";}
	}// end hiện tt xã sẽ thay đổi sửa tt khách hàng
	if (isset($_POST['id_khach_hang_sua_huyen'])) {// hiện tt xã sẽ thay đổi sửa tt khách hàng
		$id_khach_hang_sua_huyen = $_POST['id_khach_hang_sua_huyen'];
		include 'conn.php';
		$sqlhuyen = "SELECT huyen.mahuyen, huyen.caphuyen, huyen.tenhuyen FROM khachhang , huyen, tinh WHERE khachhang.id='$id_khach_hang_sua_huyen' AND khachhang.matinh=tinh.matinh AND huyen.matinh=tinh.matinh  ORDER by huyen.caphuyen, huyen.tenhuyen ASC";
		$sqlhuyenhien = mysqli_fetch_array(mysqli_query($conn,"SELECT huyen.mahuyen, huyen.caphuyen, huyen.tenhuyen FROM khachhang , huyen WHERE khachhang.id='$id_khach_hang_sua_huyen' AND khachhang.mahuyen= huyen.mahuyen ORDER by huyen.caphuyen, huyen.tenhuyen ASC"));
		$queryhuyen = mysqli_query($conn, $sqlhuyen);
		echo " <option value='".$sqlhuyenhien['mahuyen']."'>".$sqlhuyenhien['caphuyen']. $sqlhuyenhien['tenhuyen']."</option>";
		while ($rowhuyen = mysqli_fetch_array($queryhuyen)) {
			$tenhuyen = $rowhuyen['tenhuyen'];
